...
class FileSystemF2fs(FileSystemBase):
    """
    **Implements creation of f2fs filesystem**
    """
    def create_on_device(
        ...
        if label:
            ...
            #self.custom_args['create_options'].append(
                ...
                #)
            #)
        print(self.custom_args['create_options'])
        Command.run(
            ['mkfs.f2fs'] + self.custom_args['create_options'] + [device]
        )
