			...
			uint32_t keyid)
{
	...
	int botan = !!getenv("EVMCTL_USE_BOTAN");
	botan_rng_t rng;
	botan_privkey_t skey;
	if (botan) {
		uint8_t pem[256];
		FILE *fp = fopen(keyfile, "r");
		size_t pemlen;

		pemlen = fread(pem, 1, sizeof(pem), fp);
		printf("pemlen: %ld\n", pemlen);

		if (botan_rng_init(&rng, "user")) {
			perror("botan_rng_init");
			return -1;
		}
		fclose(fp);

		if (botan_privkey_load(&skey, rng, pem, pemlen, "")) {
			perror("botan_privkey_load");
			return -1;
		}
	} else {
		pkey = read_priv_pkey(keyfile, keypass, access_info, keyid);
	...
	if (botan) {
		...
		if (botan_pk_op_sign_create(&op, skey, "", 0)) {
			perror("botan_pk_op_sign_create");
			return -1;
		}

		if (botan_pk_op_sign_update(op, hash, size)) {
			perror("botan_pk_op_sign_update");
			return -1;
		}

		if (botan_pk_op_sign_output_length(op, &sigsize)) {
			perror("botan_pk_op_sign_output_length");
			return -1;
		}

		if (botan_pk_op_sign_finish(op, rng, hdr->sig, &sigsize)) {
			perror("botan_pk_op_sign_finish");
			return -1;
		}

		if (botan_pk_op_sign_destroy(op)) {
			perror("botan_pk_op_sign_destroy");
			return -1;
		}

		if (botan_privkey_destroy(skey)) {
			perror("botan_privkey_destroy");
			return -1;
		}

		if (botan_rng_destroy(rng)) {
			perror("botan_rng_destroy");
			return -1;
		}
	} else {
		st = "EVP_PKEY_CTX_new";
