...
static int paths_size = 10;
static int no_paths = 0;

static int find_helper(const char *path, int dts)
{
	struct dirent *de;
	DIR *dir;
	...
	while ((de = readdir(dir))) {
		...
		if (de->d_type == DT_DIR) {
			find_helper(de->d_name, dts);
		} else if (dts & (1 << de->d_type)) {
			if (no_paths >= paths_size) {
				paths_size *= 2;
				paths = realloc(paths, paths_size * PATH_MAX);
			}
			realpath(de->d_name, &paths[no_paths++ * PATH_MAX]);
		}
	}
...
static int find(const char *path, int dts, find_cb_t func)
{
	...
	if (!paths) {
		paths = malloc(paths_size * PATH_MAX);
	}

	find_helper(path, dts);

// https://stackoverflow.com/a/9836422
#pragma omp parallel for shared(flag)
	for (int i = 0; i < no_paths; i++) {
		if (flag)
			continue;

		if (func(&paths[i * PATH_MAX]))
			flag = 1;
	}
