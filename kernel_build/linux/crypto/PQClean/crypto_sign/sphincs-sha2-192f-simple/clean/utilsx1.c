...
void treehashx1(unsigned char *root, unsigned char *auth_path,
    ...
    for (idx = 0;; idx++) {
        unsigned char cur[2 * SPX_N]; /* Current logical node is at */
        /* index[SPX_N].  We do this to minimize the number of copies */
        /* needed during a thash */
        gen_leaf( &cur[SPX_N], ctx, idx + idx_offset,
                  info );
        ...
        for (h = 0;; h++, internal_idx >>= 1, internal_leaf >>= 1) {
            ...
            memcpy( &cur[0], left, SPX_N );
            thash( &cur[1 * SPX_N],
                   &cur[0 * SPX_N],
                   2, ctx, tree_addr);
        }

        /* We've hit a left child; save the cur for when we get the */
        /* corresponding right right */
        memcpy( &stack[h * SPX_N], &cur[SPX_N], SPX_N);
    }
}
