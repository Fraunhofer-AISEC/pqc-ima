...
class FileSystemBcachefs(FileSystemBase):
    """
    **Implements creation of bcachefs filesystem**
    """
    def create_on_device(
        ...
        if label:
            self.custom_args['create_options'].append('--fs_label')
            self.custom_args['create_options'].append(label)
        if uuid:
            self.custom_args['create_options'].append('--uuid')
            self.custom_args['create_options'].append(uuid)
        if size:
            self.custom_args['create_options'].append('--fs_size')
        ...
        print(device)
        Command.run(
            ['mkfs.bcachefs'] + self.custom_args['create_options'] + [device]
        )
